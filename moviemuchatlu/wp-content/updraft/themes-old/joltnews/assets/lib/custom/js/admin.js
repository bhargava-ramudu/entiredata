(function ($) {
    ...
    var ajaxurl = joltnews_admin.ajax_url;
    var joltNewsNonce = joltnews_admin.ajax_nonce;
    var custom_theme_file_frame;

    // Category Color
    $('#joltnews-cat-color').wpColorPicker();
    ...
    $('.twp-img-upload-button').click(function () {
        ...
        frame = wp.media({
            title: joltnews_admin.upload_image,
            button: {
                text: joltnews_admin.use_image
            },
    ...
    // Dismiss notice
    var joltnewsAjaxUrl = joltnews_admin.ajax_url;
    $('.twp-custom-setup').click(function(){
        jQuery.post(joltnewsAjaxUrl, {
            action: 'joltnews_notice_dismiss',
            security : joltNewsNonce,
        });
        $('.twp-joltnews-notice').hide();
    });

    // Getting Start action
    $('.twp-install-active').click(function () {

        $(this).closest('.twp-joltnews-notice').addClass('twp-installing');

        var data = {
            'action': 'joltnews_install_plugins',
            '_wpnonce': joltNewsNonce,
        };
    ...
    $('.theme-recommended-plugin .recommended-plugin-status').click(function () {
        ...
        $.post(ajaxurl, data, function (response) {
            ...
            var active = joltnews_admin.active
            var deactivate = joltnews_admin.deactivate
